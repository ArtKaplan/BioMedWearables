...
class TimerPage extends StatefulWidget {
  final String preselectedHike;
  const TimerPage({
    super.key,
    this.preselectedHike = 'Sentiero del Monte Cecilia',
  });
...
class _TimerPageState extends State<TimerPage> {
  ...
  void initState() {
    super.initState();
    _chosenHike = widget.preselectedHike;
    SharedPreferences.getInstance().then((sp) {
      setState(() {
        presMode = sp.getBool('presentation_mode') ?? false;
      });
    });
  ...
  void saveResult() {
    if (stopwatch.isRunning) {
      showDialog(
        // taken from https://www.dhiwise.com/post/how-to-build-customizable-pop-ups-with-flutter-dialog
        context: context,
        builder: (BuildContext context) {
          return AlertDialog(
      ...
      showDialog(
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            content: Text(
              'Are you sure that you want to save your time of ${stopwatch.elapsed + Duration(seconds: extra)} to Hike "${hikelist[index].name}"?',
            ),
            actions: <Widget>[
              TextButton(
                child: Text('Yes'),
                onPressed: () async {
                  ...
                  setState(() {
                    hikelist[index].times.add(
                      stopwatch.elapsed + Duration(seconds: extra),
                    );
                  });
                  stopwatch.reset();
                  t.cancel();
  ...
  List<String> allHikes = hikeNames();
        ...
        child: Column(
          children: [
            ...
            DropdownButton<String>(
              ...
              items:
                  allHikes.map<DropdownMenuItem<String>>((String value) {
